...
function CodePanel({ code, lang }) {
  ...
  function toggleListen() {
    ...
    // Free, built into the browser — no API call, no extra cost.
    const spoken = lines
      .map((l) => l.replace(/\/\/|#/g, ", comment:"))
      .join(". Next line. ");
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {"speechSynthesis" in window && (
            <button onClick={toggleListen}
              style={{ background: speaking ? "#dbeafe" : "#fff", border: `1px solid ${speaking ? "#3b82f6" : "#d1d5db"}`, color: speaking ? "#1d4ed8" : "#6b7280", borderRadius: 6, padding: "3px 12px", fontSize: 11, cursor: "pointer", fontWeight: 600 }}>
              {speaking ? "⏹ Stop" : "🔊 Listen"}
            </button>
          )}
          <button onClick={() => { navigator.clipboard.writeText(codeText); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
...
export default function App() {
  ...
  async function generate() {
    ...
    try {
      ...
      // which holds the real Gemini API key on the server and never
      // exposes it to the browser. One click here = exactly one Gemini request.
      ...
      setLoading(false);
      // Cooldown to stay well under this project's 5-requests-per-minute limit.
      setCooldown(15);
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "20px 16px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 20, marginBottom: 20, boxShadow: "0 2px 12px #0001" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button onClick={generate} disabled={loading || cooldown > 0 || !question.trim()} style={{
              marginLeft: "auto", padding: "10px 24px", borderRadius: 10, border: "none",
              background: (!loading && cooldown === 0 && question.trim()) ? "linear-gradient(135deg,#2563eb,#3b82f6)" : "#e2e8f0",
              color: (!loading && cooldown === 0 && question.trim()) ? "#fff" : "#94a3b8",
              fontSize: 14, fontWeight: 800, cursor: (!loading && cooldown === 0 && question.trim()) ? "pointer" : "not-allowed",
              boxShadow: (!loading && cooldown === 0 && question.trim()) ? "0 3px 12px #3b82f655" : "none",
            }}>
              {loading ? "⏳ Thinking..." : cooldown > 0 ? `⏱ Wait ${cooldown}s` : "⚡ Explain It!"}
            </button>
            {cooldown > 0 && !loading && (
              <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 6, textAlign: "right" }}>
                Short pause to stay within the free API's rate limit
              </div>
            )}
          </div>

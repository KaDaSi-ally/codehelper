...
## How requests work
Each time you click "Explain It!", the app sends exactly **one** request to your Netlify function, which internally makes exactly **two** Gemini requests (not repeated, not looped): one for the explanation/flowchart, one for the code. This keeps each individual request short, which fixed earlier truncation/JSON errors, and keeps usage predictable — no duplicate or accidental extra calls per click.
